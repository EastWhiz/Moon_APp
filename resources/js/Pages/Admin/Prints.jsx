...
export default function Dashboard({ auth }) {
    ...
    useEffect(() => {
        ...
        if (cardFilter.length != 0) {
            url.searchParams.set('q6', JSON.stringify(cardFilter));
        } else {
            url.searchParams.delete('q6');
        }
        ...
        fetch(url)
            ...
            .then((result) => {
                if (result.success == true) {
                    ...
                    setShops(result.data2.map((shop, index) => {
                        return { value: shop.id, label: shop.store_name ? shop.store_name : shop.name }
                    }));

                    const my_rows = result.data.data.map((order, index) => {
                        return {
                            id: order.id,
                            store_name: order.shop ? (order.shop.store_name ? order.shop.store_name : order.shop.name) : 'N/a',
    ...
    const handleOptionSetFilterChange = useCallback(
        (value) => {
            setOptionSetFilter(value);
            clearTimeout(timeout)
            timeout = setTimeout(() => {
                setCurrentCursor(null);
                setReload(!reload);
    ...
    const handleCardRemove = useCallback(() => { setCardFilter([]); setCurrentCursor(null); setReload(!reload); }, [tableRows]);
    ...
    const filters = [
        {
            ...
            filter: (
                <ChoiceList
                    ...
                    titleHidden
                    choices={shops}
                    selected={optionSetFilter || []}
                    onChange={handleOptionSetFilterChange}
                    allowMultiple
                />
            ),
        }, {
    ...
    if (!isEmpty(optionSetFilter)) {
        const key = "filterbyshop";
        appliedFilters.push({
            key,
            label: disambiguateLabel(key, optionSetFilter),
            onRemove: handleOptionSetFilterRemove,
        });
    }


    if (!isEmpty(cardFilter)) {
        const key = "filterbycard";
        appliedFilters.push({
            key,
            label: disambiguateLabel(key, cardFilter),
    ...
    const rowMarkup = tableRows.map(
        ({ id, store_name, order_date, order_no, customer_name, customer_email, order_total, greeting_card, order, message }, index) => (
            ...
            >
                ...
                <IndexTable.Cell>
                    {greeting_card ? (greeting_card.pairbo_product_id ? greeting_card.pairbo_product_id : 'N/a') : 'N/a'}
                </IndexTable.Cell>
                <IndexTable.Cell>
                    {order_date}
                </IndexTable.Cell>
    ...
    function disambiguateLabel(key, value) {
        switch (key) {
            case "filterbyshop":
                return `Filtered by shop: ${value.map((val) => val).join(", ")}`;
            case "filterbycard":
                return `Filtered by type: ${value.map((val) => val).join(", ")}`;
            default:
                return value;
        }
    }
